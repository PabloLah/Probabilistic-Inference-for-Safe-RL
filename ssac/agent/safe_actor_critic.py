...
class ActorCritic:
    ...
    def update(self, batch: Transition, key: jax.Array) -> dict[str, float]:
        ...
        safe_state_ratio=classifier_out["safe_state_ratio"],'''
        out = dict(
    ...
    def retrain_classifier(self, batch: Transition, key: jax.Array) -> dict[str, float]:
        (self.classifier, self.classifier_learner.state), classifier_out = update_classifier(
            batch,
            self.classifier,
            self.classifier_learner.state,
    ...
    #this function doesnt really make much sense
    def reset_classifier(self, key: jax.Array): 
        #reset classifier to retrain form scratch; for more stable safety classification
        #todo: cleaner passing of variables
        self.classifier = make_classifier(self.observation_space, self.action_space, self.config, key)
...
def update_classifier(
    ...
    def loss_fn(classifier):
        ...
        #TRY THIS: weighted losses for classifier
        '''#weight loss by class imbalance
        sum_cost = jnp.sum(batch.cost)
        unsafe_weight = jax.lax.cond(
            sum_cost <= 0,
            lambda _: 1.0,
            lambda _: 50.0,#batch.cost.shape[0] / sum_cost,
            operand=None
        )
        unsafe_states = jnp.where(batch.cost > 0, 1, 0)
        # Compute binary cross-entropy loss with weighted samples
        unweighted_losses = sigmoid_binary_cross_entropy(preds, binary_targets)
        weighted_losses = unsafe_weight * unsafe_states * unweighted_losses
        loss = unweighted_losses + weighted_losses'''

        # Compute binary cross-entropy loss; need logits
        loss = sigmoid_binary_cross_entropy(preds, binary_targets)
        return loss.mean(), (jnp.mean(jax.nn.sigmoid(preds)), jnp.mean(batch.cost), jnp.min(preds), jnp.quantile(preds, 0.1), jnp.mean(preds), jnp.mean(binary_targets)) #(preds, batch.cost) #(debugging)if we use the wandb logging in update function

    # Compute loss and gradients
    (loss, (preds_prob, costs, logit_min, logit_10_quantile, logit_mean, safe_state_ratio)), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(classifier)
    # Perform optimization step
    new_classifier, new_learning_state = learner.grad_step(classifier, grads, learning_state)
    return (new_classifier, new_learning_state), dict(loss=loss, preds_prob=preds_prob, costs=costs, logit_min=logit_min, logit_10_quantile=logit_10_quantile, logit_mean=logit_mean, safe_state_ratio=safe_state_ratio)
...
def update_safety_critics(
    ...
    def loss_fn(safety_critics):
        ...
        safety_logit = jnp.clip(safety_logit, -10, None) #clip safety_logit

        safety_prob = jax.nn.sigmoid(safety_logit)
        '''----for using fixed offline trained classifier of instance CustomMLPClassifier
